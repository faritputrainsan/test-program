...
const byte verticallatch = 6;


byte horizontalData = 2;
byte horizontalClock = 4;
const byte horizontallatch = 3;


// Penentuan lebar dan tinggi
const byte lebar = 152;
const byte tinggi = 8;
...
void setup() {
  ...
  pinMode(horizontallatch, OUTPUT);
  pinMode(verticallatch, OUTPUT);
...
void scrollingText() {
  ...
  if (millis() - scrollTiming >= scrollSpeed) {
    ...
    for (int row = 0; row < tinggi; row++) {
      ...
      for (int cols = 0; cols < lebar; cols++) {
...
void refresh() {
  ...
  for (byte row = 0; row < tinggi; row++) {
    digitalWrite(horizontallatch, LOW);
    digitalWrite(verticallatch, LOW);
...
//    for (int modul = lebar - 1; modul >= 0; modul--) {
    ...
    digitalWrite(verticallatch, HIGH); // setelah semua bit terkirim, ditampilkan ke output led
    digitalWrite(horizontallatch, HIGH);
//    }
  }
}
