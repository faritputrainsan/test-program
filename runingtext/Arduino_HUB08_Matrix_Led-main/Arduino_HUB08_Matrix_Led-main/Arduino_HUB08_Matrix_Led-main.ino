...
void showTitle(String icon, String text, bool topPosition = true) {
  ...
  ronn.setFont(C_ICON);
  ronn.scrollText_D(icon, 27, 0, 9, 10); //(string,)
...
//
  ...
  display.setBrightness(2000);     //low brightness to save energy
...
//CEAR DEMO
void clearDemo() {
  //  showTitle("4","CLEAR",false);delay(500);
...
void moveDemo() {
  ...
  char* moving[] = {"R:60", "P:3000", "U:8", "P:300", "L:14", "E:0"};
  //  R = right, P = parking/ delay, U= UP, L= left, E= end, D= down
  //  Serial.print(moving);
...
void showFont() {
  ...
  ronn.clear_R();
  ronn.setFont(B_STD);
  ronn.scrollText_LR("This is a big font demo from RONN ANIMATION - 1234567890", 0, 0, WIDTH, 20);
